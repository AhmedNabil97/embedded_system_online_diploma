...
int reverse_list()
{
	if(Pt_head_list)                                                              // check if the list exist
	{
		struct Sstudents* p_current  = Pt_head_list;
		struct Sstudents* p_previous = p_current;                               //p_previous to reverse the addrsses of pnext in each node
     while(p_current->Pnextstudent)
     {
    	 p_previous = p_current;
    if	 (p_previous->Pnextstudent->Pnextstudent)
    {
    	p_previous->Pnextstudent->Pnextstudent = p_previous;

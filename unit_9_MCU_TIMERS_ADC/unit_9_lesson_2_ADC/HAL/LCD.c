...
lcd_position(int x,int y)
{
	...
	for(i=0; i<y; i++)
	{
		lcd_send_command(lcd_crouser_shift_right);
	}	
...
{
	int i;
	lcd_send_command(lcd_hide_crouser);
lcd_position(x,y);
for(i=0; i<z; i++)
{
lcd_display_char(' ');
...
lcd_position(x,y);
lcd_send_command(lcd_restore_crouser);
}  
